...
(async () => {
    ...
    async function init() {
        ...
        const _st = {
            ...
            consent:  await GM_getValue('nostrcomments_consent',  false),
            keybackup: await GM_getValue('nostrcomments_keybackup', false),
            supporter: await GM_getValue('nostrcomments_supporter', false),
            lastseen: await GM_getValue('nostrcomments_lastseen', null),
            mutewords: await GM_getValue('nostrcomments_mutewords', null),
            signer:    await GM_getValue('nostrcomments_signer',    null),
            nip05:     await GM_getValue('nostrcomments_nip05',     false),
            pwoffered: await GM_getValue('nostrcomments_pwoffered', false),
            btnpos:    await GM_getValue('nostrcomments_btnpos',    null),
            notifs:    await GM_getValue('nostrcomments_notifs',    null),
            backupasked: await GM_getValue('nostrcomments_backupasked', 0),
            relaymig:  await GM_getValue('nostrcomments_relaymig',  0),
            widepublish: await GM_getValue('nostrcomments_widepublish', true),
            theme:     await GM_getValue('nostrcomments_theme',     null),
            autoimg:   await GM_getValue('nostrcomments_autoimg',   true),
            clienttag: await GM_getValue('nostrcomments_clienttag', true),
        };
        ...
        let isSupporter = _st.supporter === true;
        // 'local' | 'nip07' — which key signs. Null means: decide from what is available.
        let signerPref = _st.signer === 'nip07' || _st.signer === 'local' ? _st.signer : null;
        // Per origin, so moving it out of the way on one site does not move it everywhere.
        const _btnPosAll = (_st.btnpos && typeof _st.btnpos === 'object') ? _st.btnpos : {};
        let btnCorner = /^(tl|tr|bl|br)$/.test(_btnPosAll[location.origin]) ? _btnPosAll[location.origin] : 'br';
        const saveBtnCorner = () => { try { _btnPosAll[location.origin] = btnCorner; GM_setValue('nostrcomments_btnpos', _btnPosAll); } catch(e) {} };
        ...
        let themePref = _st.theme === 'dark' || _st.theme === 'light' ? _st.theme : null;
        if (!themePref) {
            try {
                const legacy = localStorage.getItem('nostrcomments_theme');
                if (legacy === 'dark' || legacy === 'light') {
                    themePref = legacy;
                    GM_setValue('nostrcomments_theme', legacy);
                }
        ...
        themeBtn.onclick = () => {
            ...
            GM_setValue('nostrcomments_theme', themePref);
            applyTheme();
        };
        ...
        s.getElementById('privkey-import-btn').onclick = async () => {
            ...
            try {
                await GM_setValue('nostrcomments_privkey', priv); await GM_setValue('nostrcomments_keybackup', true);
        ...
        privkeyRotate.onclick = async () => {
            ...
            try {
                ...
                await GM_setValue('nostrcomments_keybackup', false);
        ...
        privkeyDelete.onclick = async () => {
            ...
            GM_deleteValue('nostrcomments_keybackup');
        ...
        privkeyBackup.onchange = () => {
            ...
            GM_setValue('nostrcomments_keybackup', keyBackedUp);
            updateBackupUI();
        };
        ...
        pwBtn.onclick = async () => {
            if (encPriv) {
                ...
                await GM_setValue('nostrcomments_privkey', enc);
        ...
        // A userscript runs in the page, so this is a live read rather than a remembered answer —
        // which also means it is correct the moment somebody opens the panel, with nothing to
        // observe or invalidate.
        const signerInPage = () => !!_nip07Wallet();
        ...
        // explicitly enables NostrComments here.
        ...
        Object.assign(consentOverlay.style, {display:'none',position:'absolute',inset:'0',background:'inherit',borderRadius:'16px',padding:'30px 26px',flexDirection:'column',justifyContent:'center',zIndex:'20',boxSizing:'border-box'});
        {
            ...
            _cBtn.onclick = async () => {
                hasConsent = true;
                try { await GM_setValue('nostrcomments_consent', true); } catch(e) {}
        ...
        let pwOffered = _st.pwoffered === true;
        let backupAskedAt = Number(_st.backupasked) || 0;
        const rememberBackupAsked = () => { try { GM_setValue('nostrcomments_backupasked', Date.now()); } catch(e) {} };
        ...
        async function offerEncryption(privHex) {
            ...
            GM_setValue('nostrcomments_pwoffered', true);
        ...
        let RELAYS = (() => {
            try {
                ...
                const done = _st.relaymig >= RELAY_MIGRATION;
        ...
        if (_st.relaymig !== RELAY_MIGRATION) {
            try {
                GM_setValue('nostrcomments_relaymig', RELAY_MIGRATION);
                if (relaysNeedSave) GM_setValue('nostrcomments_relays', RELAYS);
            } catch(e) {}
        }
        ...
        let publishWide = _st.widepublish !== false;
        ...
        let labelClient = _st.clienttag !== false;
        ...
        async function renderDisabledList() {
            ...
            arr.forEach(origin => {
                ...
                enableBtn.onclick = async () => {
                    const cur = (await GM_getValue('nostrcomments_disabled', null)) || [];
                    const next = (Array.isArray(cur) ? cur : []).filter(o => o !== origin);
                    await GM_setValue('nostrcomments_disabled', next);
                    renderDisabledList();
        ...
        const openSettings = async () => {
            ...
            if (localWallet || encPriv) {
                keypairSection.style.display = 'block';
                if (localWallet) {
                    const _saved = await GM_getValue('nostrcomments_privkey', '');
                    if (!_isEncPriv(_saved)) setPrivHex(_saved || '');
                } else {
                    _privHex = '';
                }
                hidePrivkey();
        ...
        const comments = [];
        let _lastSeenMap = (_st.lastseen && typeof _st.lastseen === 'object') ? _st.lastseen : {};
        let _newSince = 0;
        ...
        let autoImages = _st.autoimg !== false;
        ...
        autoImgToggle.onchange = () => {
            ...
            GM_setValue('nostrcomments_autoimg', autoImages);
            render();
        ...
        nip05Toggle.onchange = () => {
            ...
            GM_setValue('nostrcomments_nip05', nip05Check);
        ...
        widepubToggle.onchange = () => {
            ...
            GM_setValue('nostrcomments_widepublish', publishWide);
        ...
        clienttagToggle.onchange = () => {
            ...
            GM_setValue('nostrcomments_clienttag', labelClient);
        ...
        let muteWords = Array.isArray(_st.mutewords) ? _st.mutewords.map(w => String(w).toLowerCase()) : [];
        function saveMuteWords() { GM_setValue('nostrcomments_mutewords', muteWords); }
        ...
        let notifLog = Array.isArray(_st.notifs) ? _st.notifs.slice(0, 50) : [];
        const saveNotifs = () => { try { GM_setValue('nostrcomments_notifs', notifLog.slice(0, 50)); } catch(e) {} };
        ...
        // --- NIP-07 / WebLN provider access ---
        // In Firefox a userscript runs in a sandbox with Xray vision, so window.nostr / window.webln
        // (injected by nos2x / Alby into the real page window) are only reachable via unsafeWindow.
        // Wrap the provider so the event object we pass is cloned into the page compartment and the
        // returned strings are read back cleanly across the boundary. (Chrome has no Xray, so this
        // transparently falls back to plain window access.)
        const _pageWin = (typeof unsafeWindow !== 'undefined' && unsafeWindow) ? unsafeWindow : window;
        const _SIGNER_TIMEOUT_MS = 60000;
        function _signerTimeout(p) {
            return new Promise((resolve, reject) => {
                const t = setTimeout(() => reject(new Error('NIP-07 timeout')), _SIGNER_TIMEOUT_MS);
                Promise.resolve(p).then(
                    v => { clearTimeout(t); resolve(v); },
                    e => { clearTimeout(t); reject(e); });
            });
        }
        function _nip07Wallet() {
            const n = _pageWin.nostr;
            if (!n) return null;
            return {
                getPublicKey: async () => {
                    const p = '' + (await n.getPublicKey());
                    if (!_isPubkey(p)) throw new Error('signer returned an invalid public key');
                    return p;
                },
                signEvent: async (ev) => {
                    const arg = (typeof cloneInto === 'function') ? cloneInto(ev, _pageWin) : ev;
                    // The extensions bound this wait in the bridge; here the provider is called
                    // straight, so a signer that stopped answering left the Post button disabled
                    // until the tab was reloaded. Same minute, and the same reason for it: the wait
                    // is for a person reading a prompt, but it still has to end.
                    const r = await _signerTimeout(n.signEvent(arg));
                    return { ...ev, id: '' + r.id, pubkey: '' + r.pubkey, sig: '' + r.sig };
                }
            };
        }
        ...
        // A stored key and a NIP-07 signer could both be present, and which one won was implicit —
        // and differed from the extension builds. The choice is now explicit; with no choice made
        // this build keeps its previous behaviour so nobody's identity changes under them.
        ...
        async function carriesPrivateKey(text) {
            ...
            try {
                const k = await GM_getValue('nostrcomments_privkey', null);
        ...
        function getWallet() {
            if (signerPref === 'nip07') return _nip07Wallet();
            if (signerPref === 'local') return localWallet || _nip07Wallet();
            return _nip07Wallet() || localWallet;
        }

        const _isPubkey = v => typeof v === 'string' && /^[0-9a-f]{64}$/i.test(v);

        // This build reads window.nostr straight out of the page rather than through a bridge, so
        // whatever the page puts there is what signs. That is inherent to a userscript and cannot
        // be closed off — but it need not be believed either. A signed event carries the pubkey
        // that actually signed it, and producing one for a key you do not hold is the thing the
        // protocol rests on being impossible. So the identity comes from the signature rather than
        // from getPublicKey, and a mismatch is corrected rather than trusted. A signer that
        ...
        function saveSigner() { GM_setValue('nostrcomments_signer', signerPref); }
        ...
        async function chooseSigner(pref) {
            if (pref === 'nip07') {
                if (!_nip07Wallet()) return showMsg('No Nostr signer found — install Alby or nos2x first');
                ...
                // you had no key at all — while it sat in storage, untouched.
                const _saved = await GM_getValue('nostrcomments_privkey', '');
        ...
        async function lnurlPay({lud16, pubkey, eventId, amount, target, successMsg}) {
            ...
            if (_pageWin.webln) {
                try { await _pageWin.webln.enable(); await _pageWin.webln.sendPayment(pr); showMsg(successMsg); return true; } catch(e) {}
            }
        ...
        function saveSupporter() { GM_setValue('nostrcomments_supporter', true); }
        (() => {
...
// A comment that reached exactly one relay is published, so nothing here is an error — but
        ...
        function startNetwork() {
            ...
            // Load the stored key, or not, according to what the user actually chose. Choosing
            // "Key stored here" and then reloading with a signer present used to connect you as the
            // signer's identity instead, silently, with your own key untouched in storage. Only
            // when nothing has been chosen does the presence of a signer decide.
            (async () => {
                const saved = _st.privkey;
                const useLocal = signerPref === 'local' ? true
                               : signerPref === 'nip07' ? false
                               : !_pageWin.nostr;
                if (saved && !_isEncPriv(saved) && useLocal) { localWallet = await makeLocalWallet(saved); connect(); }
                ...
                if (saved && !_isEncPriv(saved)) offerEncryption(saved);
            })();

            // Auto-connect to a NIP-07 signer if one becomes available
            // Keep looking for a while rather than giving up the first time: a signer extension
            // may not have injected window.nostr yet when the first look happens.
            let _signerTries = 0;
            const connectTimer = setInterval(() => {
                if (myPub) { clearInterval(connectTimer); return; }
                if (!_pageWin.nostr && !localWallet) {
        ...
        function markPageSeen() {
            ...
            GM_setValue('nostrcomments_lastseen', _lastSeenMap);
        }
